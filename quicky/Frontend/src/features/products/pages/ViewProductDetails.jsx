                ...
                <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
                    ...
                    <div className="w-full lg:w-[60%] flex flex-col md:flex-row gap-6">
                        ...
                        {product.images && product.images.length > 1 && (
                            <div className="hidden md:flex flex-col gap-4 w-24 flex-shrink-0 max-h-[600px] overflow-y-auto scrollbar-hide pr-2">
                                {product.images.map((img, idx) => (
                                    <button
                                        key={idx}
                                        onClick={() => setActiveImage(idx)}
                                        className={`relative flex-shrink-0 w-full aspect-square rounded-2xl overflow-hidden border-2 transition-all duration-300 ${activeImage === idx ? 'border-[#f97316] scale-95 shadow-lg' : 'border-transparent opacity-60 hover:opacity-100 hover:scale-105'}`}
                                    >
                                        <img
                                            src={img.url || img}
                        ...
                        <div className="flex-1 space-y-6">
                            {/* Main Image Swiper Slot */}
                            <div className="relative aspect-[4/5] md:aspect-square lg:aspect-[4/5] bg-white rounded-3xl overflow-hidden border border-dashed border-[#8c7164]/30 p-4 group">
                                            ...
                                            {product.images.length > 1 && (
                                                <>
                                                    <button
                                                        ...
                                                        className="absolute left-4 top-1/2 -translate-y-1/2 p-3 bg-white/80 backdrop-blur-md rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-white text-slate-800 hover:text-[#f97316] z-10"
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-white/80 backdrop-blur-md rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-white text-slate-800 hover:text-[#f97316] z-10"
                                                    >
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                                                    ...
                                                    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 z-10">
                                                        {product.images.map((_, idx) => (
                                                            <div
                                                                key={idx}
                                                                className={`h-1.5 rounded-full transition-all duration-300 ${activeImage === idx ? "w-6 bg-[#f97316]" : "w-1.5 bg-white/60"}`}
                                                            />
                                                        ))}
                                                    </div>
                                                </>
                            ...
                            {product.images && product.images.length > 1 && (
                                <div className="flex md:hidden gap-4 overflow-x-auto pb-2 scrollbar-hide text-white">
                                    {product.images.map((img, idx) => (
                                        <button
                                            key={idx}
                                            onClick={() => setActiveImage(idx)}
                                            className={`relative flex-shrink-0 w-20 aspect-square rounded-2xl overflow-hidden border-2 transition-all duration-300 ${activeImage === idx ? 'border-[#f97316] scale-95 shadow-lg' : 'border-transparent opacity-60 hover:opacity-100 hover:scale-105'}`}
                                        >
                                            <img
                                                src={img.url || img}
                                                alt={`${product.title} view ${idx + 1}`}
                        ...
                        <div className="sticky top-28 space-y-10">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-baseline gap-2">
                                    ...
                                    <span className="text-sm text-slate-400 font-medium line-through decoration-orange-600">
                            ...
                            <div className="space-y-6 pt-4">
                                <div className="flex flex-col sm:flex-row gap-4">
                                    <button onClick={() => {
                                        checkUser()
                                    }} className="flex-1 bg-gradient-to-br from-[#9d4300] to-[#f97316] text-white py-5 rounded-full text-base font-black transition-all hover:-translate-y-1 hover:shadow-[0_20px_40px_-10px_rgba(157,67,0,0.4)] active:scale-95 flex items-center justify-center gap-3">
                                    ...
                                    <button onClick={() => {
                                        checkUser()
                                    }} className="flex-1 bg-[#0b1c30] text-white py-5 rounded-full text-base font-black transition-all hover:bg-slate-800 hover:-translate-y-1 hover:shadow-[0_20px_40px_-10px_rgba(11,28,48,0.3)] active:scale-95 flex items-center justify-center">
                                        Buy Now
                                    </button>
                                </div>
